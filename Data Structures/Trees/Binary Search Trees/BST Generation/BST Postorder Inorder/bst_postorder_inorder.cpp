...
Node *buildBstTree(int postorder[], int inorder[], int start, int end)
{
    static int idx = 5;
    if(start > end)
    {
    ...
    node->right = buildBstTree(postorder, inorder, pos+1, end);
    node->left = buildBstTree(postorder, inorder, start, pos-1);
    return node;
}
...
int main()
{
    ...
    Node* root = buildBstTree(postorder, inorder, 0, n-1);
    checkInorder(root);
    return 0;
}